...
int main() {
    ...
    while ( true ) {
        ...
        string cmd_upper = to_upper(command_to_be_executed);

        // Find the command in COMMANDS
        const Command* matched_command = nullptr;
        for (const auto &cmd : COMMANDS) {
            if (to_upper(cmd.str) == cmd_upper) {
                matched_command = &cmd;
                break;
            }
        }
        ...
        if (pieces.size() - 1 != matched_command->parameter_number) {
            cout << "Error: wrong number of parameters." << endl;
            continue;
        }

        // Exit commands
        if (matched_command->is_exit) {
            cout << GREETING_AT_END << endl;
            break;
        }
        ...
        for (size_t i = 1; i < pieces.size(); ++i) {
            double val;
            if (!string_to_double(pieces[i], val)) {
                valid = false;
                break;
            }
        ...
        // Execute the action
        double result = matched_command->action(operands[0], operands[1]);
        cout << result << endl;
